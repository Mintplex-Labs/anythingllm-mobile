...
import { getOrigin } from "@/utils/formatters";
...
export default {
    ...
    config: {},
    execute: async function (url: string, streamEmitter: (event: IStreamEvent, data: any) => void): Promise<string> {
        try {
            ...
            validUrl = new URL(validUrl).toString();
            const scrapeResult = await webscraper.scrape(validUrl);
